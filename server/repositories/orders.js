...
const addBatches = async (batches) => {
  const query = format(
    "INSERT INTO batches (order_id, sizes_id, quantity, removed) VALUES %L",
    batches
  )
...
const getBuyersOrders = async (buyers_id) => {
  const query =
    "SELECT event_id, event_start, event_endtime, json_build_object( 'id', market_id, 'address', market_address, 'city', market_city, 'name', market_reko_name ) AS market, price, order_id, order_date, products FROM ( SELECT ( SELECT COALESCE(SUM(sizes.unit * products.unit_price), 0) FROM products INNER JOIN sizes ON sizes.product_id = products.id INNER JOIN batches ON batches.sizes_id = sizes.id INNER JOIN products_events ON products_events.id_product = products.id WHERE batches.removed = false AND products.sellers_id = $1 AND products_events.id_event = e.id ) AS price, e.id AS event_id, e.start AS event_start, e.endtime AS event_endtime, markets.address AS market_address, markets.city AS market_city, markets.id AS market_id, reko_areas.name AS market_reko_name, orders.id AS order_id, orders.created_at AS order_date, json_agg( json_build_object( 'quantity', batches.quantity, 'product_name', products.name, 'product_id', products.id, 'product_image_url', products.image_url, 'size', sizes.unit, 'price', sizes.unit * products.unit_price, 'type', products.type, 'unit_price', products.unit_price, 'description', products.description, 'organic', products.organic, 'removed', batches.removed, 'seller_id', sellers.id, 'seller_name', sellers.name, 'seller_firstname', users.firstname, 'seller_lastname', users.lastname, 'seller_image_url', sellers.image_url ) ) AS products from orders INNER JOIN batches ON orders.id = batches.order_id INNER JOIN sizes ON sizes.id = batches.sizes_id INNER JOIN products ON products.id = sizes.product_id INNER JOIN buyers ON buyers.id = orders.buyers_id INNER JOIN events AS e ON e.id = orders.event_id INNER JOIN markets ON markets.id = e.market_id INNER JOIN reko_markets ON reko_markets.market_id = markets.id INNER JOIN reko_areas ON reko_areas.id = reko_markets.areas_id INNER JOIN sellers ON products.sellers_id = sellers.id INNER JOIN users ON users.id = sellers.id WHERE buyers.id = $1 GROUP by ( e.id, e.endtime, e.start, markets.address, markets.id, reko_areas.name, sellers.id, sellers.image_url, users.firstname, users.lastname, sellers.name, orders.id ) ) AS res"
...
const removeSellersOrder = async (seller_id, order_id) => {
  await db.query(
    "UPDATE batches SET removed=true FROM sizes, products WHERE batches.order_id=$1 AND batches.sizes_id = sizes.id AND sizes.product_id = products.id AND products.sellers_id=$2;",
    [order_id, seller_id]
  )
}
...
const removeSizeFromSellersOrder = async (order_id, size_id) => {
  await db.query(
    "UPDATE batches SET removed=true FROM sizes, products WHERE batches.order_id=$1 AND batches.sizes_id = $2",
    [order_id, size_id]
  )
}
...
const removeProductFromSellersOrder = async (order_id, product_id) => {
  await db.query(
    "UPDATE batches SET removed=true FROM sizes, products WHERE batches.order_id=$1 AND sizes.product_id = products.id AND products.id = $2",
    [order_id, product_id]
  )
}
...
module.exports = {
  ...
  getBuyersOrders,
  removeSellersOrder,
  removeProductFromSellersOrder,
  getSellersOrder,
  getSellersOrderBySize,
  getSellersOrderByProduct,
  removeSizeFromSellersOrder,
}
