...
const getSellersOrders = async (sellers_id) => {
  const query = "SELECT  event_id, event_endtime, json_agg(json_build_object('order_id', order_id, 'users_firstname', users_firstname, 'users_lastname', users_lastname, 'price', price, 'user_orders', orders)) AS events_orders FROM (SELECT orders.id, users.firstname AS users_firstname, users.lastname AS users_lastname, (SELECT COALESCE(SUM(sizes.unit*products.unit_price),0) FROM products INNER JOIN sizes ON sizes.product_id = products.id INNER JOIN batches ON batches.sizes_id = sizes.id INNER JOIN products_events ON products_events.id_product = products.id WHERE batches.removed=false AND products.sellers_id=$1 AND products_events.id_event=e.id) AS price, orders.id AS order_id, e.id AS event_id, e.endtime AS event_endtime, json_agg(json_build_object('size_id', s.id,'quantity', b.quantity, 'product_name', p.name, 'size', s.unit, 'price', s.unit*p.unit_price, 'type', p.type, 'unit_price', p.unit_price, 'removed', b.removed)) AS orders from orders INNER JOIN batches AS b ON orders.id = b.order_id INNER JOIN sizes AS s ON s.id = b.sizes_id INNER JOIN products AS p ON p.id = s.product_id INNER JOIN buyers ON buyers.id = orders.buyers_id INNER JOIN users ON users.id = buyers.id INNER JOIN events AS e ON e.id = orders.event_id WHERE p.sellers_id=$1 GROUP BY (users.firstname, users.lastname, e.id, e.endtime, orders.id)) AS res GROUP BY (event_id, event_endtime)"
